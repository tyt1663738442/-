          ...
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">东方财富实时数据</p>
              </div>
            </div>
      ...
      <div className="border-b border-stock-border">
        <div className="max-w-7xl mx-auto px-4">
          <nav className="flex gap-1">
            <TabButton active={activeTab === 'market'} onClick={() => setActiveTab('market')} icon={<Activity className="w-4 h-4" />} label="市场行情" />
            <TabButton active={activeTab === 'bigorder'} onClick={() => setActiveTab('bigorder')} icon={<Zap className="w-4 h-4" />} label="大单监控" badge={bigOrders.length || undefined} />
            <TabButton active={activeTab === 'daban'} onClick={() => setActiveTab('daban')} icon={<Target className="w-4 h-4" />} label="打板精选" badge={dabanCandidates.length || undefined} />
          </nav>
        </div>
      </div>
...
function TabButton({ active, onClick, icon, label, badge }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string; badge?: number }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-4 py-3 text-sm font-medium border-b-2 transition-colors ${active ? 'border-blue-500 text-blue-400 bg-blue-500/10' : 'border-transparent text-gray-400 hover:text-white'}`}
    >
      {icon}
      <span>{label}</span>
      {badge !== undefined && badge > 0 && (
        <span className={`px-2 py-0.5 text-xs rounded-full ${active ? 'bg-blue-500' : 'bg-gray-700'}`}>{badge}</span>
      )}
    </button>
  )
}
