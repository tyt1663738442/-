...
# ============== 消息类型关键词 ==============
NEWS_TYPE_KEYWORDS = {
    'policy': ['国务院', '发改委', '央行', '财政部', '证监会', '工信部', '通知', '意见', '规划', '纲要', '决定', '条例', '办法', '国常会', '政治局', '中央', '部委', '政府', '中共中央', '全国人大'],
    'industry': ['行业', '板块', '产能', '供需', '涨价', '市场', '产业链', '供应链', '景气', '周期'],
    'macro': ['美国', '美联储', '欧洲', '欧盟', 'G7', 'OPEC', '英国', '日本', '德国', '法国', '制裁', '关税', '全球', '国际', '原油', '黄金', '美股', '港股'],
    'stock': ['公司', '协议', '合同', '收购', '定增', '回购', '增持', '减持', '业绩', '分红', '股权', '重组', '更名', '摘牌', '公告'],  # 移除了无股票代码归属的"公告"关键词
}
...
def analyze_news(news_item: dict) -> dict:
    ...
    # 1. 判断消息类型（有个股代码归属的公告，强制归类为个股，不归政策）
    stock_code_api = news_item.get('stock_code_api', '')
    if stock_code_api:
        # 东方财富个股公告：有明确股票代码，只属于个股，不属于政策
        ntype = 'stock'
    else:
